...
if device == "cpu":
    model = torch.load(
        os.path.join("models", "deepranknet_pixsy_data_unfreeze_avg_maxpool_64.model"),
        map_location=device,
    )
else:
    model = torch.load(
...
parser.add_argument("--Type", "-T", help="Image type: enter 'photos' or 'matches'")
...
def extract_features(im_type):

    folder_name = os.path.join("images", im_type)
    ...
    for f in files:
        ...
        idx.append(f)  # using the image filename as the index for the feature
    ...
    # df.to_csv(os.path.join("image_features", im_type, "features.csv"), index=True)
    print("saved features for the images in {}".format(folder_name))
    print(
